...
// 缓存设备信息，避免频繁获取
lazy_static! {
    static ref DEVICE_INFO_CACHE: Arc<Mutex<Option<(DeviceInfo, SystemTime)>>> = Arc::new(Mutex::new(None));
    static ref HARDWARE_FINGERPRINT_CACHE: Arc<Mutex<Option<(String, SystemTime)>>> = Arc::new(Mutex::new(None));
    static ref MAC_ADDRESS_CACHE: Arc<Mutex<Option<(Option<String>, SystemTime)>>> = Arc::new(Mutex::new(None));
}
...
impl DeviceInfo {
    pub fn get() -> Self {
        ...
        if let Some((cached_info, cache_time)) = DEVICE_INFO_CACHE.lock().unwrap().as_ref() {
            if SystemTime::now().duration_since(*cache_time).unwrap_or(Duration::from_secs(0)) < CACHE_EXPIRY_DURATION {
...
fn get_mac_address_internal() -> Option<String> {
    ...
    for interface in interfaces {
        // 跳过回环接口
        if interface.name.to_lowercase().contains("loopback") || interface.name.to_lowercase() == "lo" {
            continue;
        }
...
fn generate_hardware_fingerprint() -> String {
    ...
    // 检查缓存中是否有有效的硬件指纹
    if let Some((cached_fingerprint, cache_time)) = HARDWARE_FINGERPRINT_CACHE.lock().unwrap().as_ref() {
        if SystemTime::now().duration_since(*cache_time).unwrap_or(Duration::from_secs(0)) < CACHE_EXPIRY_DURATION {
...
fn generate_hardware_fingerprint_internal() -> String {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // 使用更简单的命令获取CPU ID
        let output = Command::new("powershell")
            .args(["-Command", "(Get-CimInstance -Class Win32_Processor).ProcessorId"])
            .output();
