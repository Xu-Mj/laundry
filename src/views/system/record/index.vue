<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="本次使用积分数量" prop="identify">
        <el-input
          v-model="queryParams.identify"
          placeholder="请输入本次使用积分数量"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="recordList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="主键，自增ID" align="center" prop="id" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="recordRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="本次使用积分数量" prop="identify">
          <el-input v-model="form.identify" placeholder="请输入本次使用积分数量" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const data = reactive({
  ...
  rules: {
    ...
    identify: [
      { required: true, message: "本次使用积分数量不能为空", trigger: "blur" }
    ],
    createTime: [
      { required: true, message: "积分使用时间不能为空", trigger: "blur" }
    ]
  }
});
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除积分使用记录编号为"' + _ids + '"的数据项？').then(function() {
