...
function refreshCacheNames() {
  getCacheNames();
  proxy.notify.success("刷新缓存列表成功");
}

/** 清理指定名称缓存 */
function handleClearCacheName(row) {
  clearCacheName(row.cacheName).then(response => {
    proxy.notify.success("清理缓存名称[" + row.cacheName + "]成功");
    getCacheKeys();
  });
}
...
function refreshCacheKeys() {
  getCacheKeys();
  proxy.notify.success("刷新键名列表成功");
}

/** 清理指定键名缓存 */
function handleClearCacheKey(cacheKey) {
  clearCacheKey(cacheKey).then(response => {
    proxy.notify.success("清理缓存键名[" + cacheKey + "]成功");
    getCacheKeys();
  });
}
